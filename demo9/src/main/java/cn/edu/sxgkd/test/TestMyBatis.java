...
public class TestMyBatis {
    ...
    public static void testQueryStudentAndCardBusinessByStuNo() throws IOException {
        String resource = "conf.xml";
        Reader reader = Resources.getResourceAsReader(resource);
        SqlSessionFactory sessionFactory
                = new SqlSessionFactoryBuilder().build(reader);
        SqlSession session = sessionFactory.openSession();
        IStudentMapper studentMapper = session.getMapper(IStudentMapper.class);
        StudentAndCardBusiness studentAndCard = studentMapper.queryStudentAndCardBusinessByStuNo(32);
        System.out.println(studentAndCard);
        session.close();
    }

    public static void queryStudentAndCardByStuNoWithResultMap() throws IOException {
        String resource = "conf.xml";
        Reader reader = Resources.getResourceAsReader(resource);
        SqlSessionFactory sessionFactory
                = new SqlSessionFactoryBuilder().build(reader);
    ...
    public static void queryClassAndStudentByClassId() throws IOException {
        String resource = "conf.xml";
        Reader reader = Resources.getResourceAsReader(resource);
        SqlSessionFactory sessionFactory
                = new SqlSessionFactoryBuilder().build(reader);
        SqlSession session = sessionFactory.openSession();
        IStudentMapper studentMapper = session.getMapper(IStudentMapper.class);
        StudentClass stuClass = studentMapper.queryClassAndStudentByClassId(1);
    ...
    public static void queryStudentByNoTwiceWithUpdate() throws IOException {
        ...
        Student stu = new Student(7,"李四",18,"就业班");
        studentMapper.updateStudentByNo(stu);
        session.commit();
        Student student2 = studentMapper.queryStudentByNo(7);
        session.close();
    }

    public static void main(String[] args) throws IOException {
        queryClassAndStudentByClassId();
    }
